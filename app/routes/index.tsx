...
function Paragraph ({ children }: {children: React.ReactNode}) {
  return (
    <p className="text-2xl max-w-5xl mb-4">{children}</p>
  )
}
...
function IntroLinkButton ({ children, to }: {children: string, to: string}) {
  return (
    <Link className="bg-sky-400 hover:bg-sky-600 hover:text-white mr-8 px-4 py-4 text-lg ease-in-out" to={to}>{children}</Link>
  )
}
  ...
  return (
    <>
      <div className="max-w-screen-xl mx-auto py-4 px-4">
        <Paragraph>
          Howdy! During the day, I am a systems engineer at <ExternalLink href="https://www.autozone.com/">AutoZone</ExternalLink>, primarily leading a small team of React developers on the B2C web application.
        </Paragraph>
        <Paragraph>
